...
class Memo(agent.Wrapper):
    def __init__(self, agt):
        agent.Wrapper.__init__(self, agt)
    ...
    def update(self, crt_step, max_step, info_in=None, **kwargs):
        if info_in is None:
            info_in = {}
...
class EnvToMemObsStack(agent.Wrapper):
    def __init__(self, agt):
        agent.Wrapper.__init__(self, agt)
        # self.attr.obs_stack = np.zeros([agt.attr.args.env_num]+[agt.attr.args.stack_num]+list(
        # agt.attr.stack_shape), dtype=agt.attr.stack_dtype)
        self.attr.obs_stack = np.zeros(
            [agt.attr.init_obs.shape[0]] + [agt.args.stack_num] + list(agt.attr.init_obs.shape[1:]),
            dtype=agt.attr.init_obs.dtype)
