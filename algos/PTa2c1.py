...
class Policy(torch.nn.Module):
    def __init__(self, obs_space, act_space, args, device):
        ...
        if net != None:self.base = net_wrapper(net,inputs_shape)
        else: ### use models end
        ...
        if   act_space.__class__.__name__ == "Discrete":    self.dist = dists.Categorical(self.base.num_outputs, act_space.n)
        elif act_space.__class__.__name__ == "Box":         self.dist = dists.DiagGaussian(self.base.num_outputs, act_space.shape[0])
        elif act_space.__class__.__name__ == "MultiBinary": self.dist = dists.Bernoulli(self.base.num_outputs, act_space.shape[0])
...
class Algo(algos.PTAlgo):
    ...
    def update(self, crt_step, max_step, info_in):
        if self.args.env_mode=='supervise': ###
            if len(info_in['mb_info'][0]['labels'])==0:
                return
            else:
                if 0:
                    labels = info_in['mb_info'][0]['labels']
                    labels = np.array(labels)
                    target = torch.from_numpy(labels).cuda()
                    inputs = info_in['mb_obs'][0]
                    inputs = np.array(inputs)
                    inputs = torch.from_numpy(inputs).cuda()
                    _, actor_features = self.model.base(inputs)
                    x = self.model.dist(actor_features).probs # equal softmax
                    x = torch.log(x)
                    action_loss = F.nll_loss(x, target)
                else:
                    labels = []
                    for info in info_in['mb_info']:
                        labels.append(info['labels'][:,np.newaxis])
                    labels = np.array(labels)
                    ...
                    _, action_log_probs, _ = self.model.get_loss(self.mb_obs_batch,target)
                    action_log_probs = action_log_probs.squeeze()
                    action_loss = -action_log_probs.mean()
                    #print('Loss:',action_loss.item())
        ...
        self.returns   = torch.zeros(info_in['mb_rew'].shape[0]+1, *info_in['mb_rew'].shape[1:], 1).to(self.device).float()
        if 0:
            print('update')
        ...
        torch.cuda.synchronize()
        if self.args.opt!='Acktr': torch.nn.utils.clip_grad_norm_(self.model.parameters(),self.args.max_grad_norm)
        self.optimizer.step()
