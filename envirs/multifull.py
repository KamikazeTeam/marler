import numpy as np
import gym, easydict, cv2, random, json
...
def env_maker(env_name, ienv, env_seed, args):
    def __make_env():
        try:
            ...
            print('No module named envirs.preprocess.'+args.env_type+'-'+args.env_name+', using default preprocess (None).')
        ...
        env = ss.black_death_v2(env) # when an agent die, other teammate's attack_penalty and opponent's attack_opponent_reward are increased...
...
class Monitor_before(gym.Wrapper):
    def __init__(self, env, args):
        ...
        self.r_step  = 0
        self.fontdict = {'fontFace':cv2.FONT_HERSHEY_SIMPLEX, 'fontScale':0.5, 'color':(0,255,0), 'thickness':1, 'lineType':cv2.LINE_4}
    def reset(self):
        obs = self.env.reset()
    ...
    def render(self,mode):
        ...
        frame = self.env.render(mode)
        frame = self.env.render(mode)
        frame = cv2.resize(frame, (int(frame.shape[1]*self.zoom_in),int(frame.shape[0]*self.zoom_in)), interpolation = cv2.INTER_AREA)
